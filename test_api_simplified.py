...
from video_system.utils.session_manager import get_session_manager

session_service = get_session_manager()
from fastapi.testclient import TestClient


def test_api_imports():
    """Test that the simplified API imports only ADK components."""
    from video_system.api import endpoints as api_simplified
...
async def test_session_creation():
    ...
    session = await session_service.create_session(
        app_name="test-app", user_id="test-user", state={"test": "data"}
    )
...
def test_api_endpoints():
    ...
    assert (
        "current_stage" in status_data["stage"]
        or status_data["stage"] == "initializing"
    )
