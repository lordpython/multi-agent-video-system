...
from video_system.shared_libraries.adk_session_models import VideoGenerationStage, SessionMetadata
...
async def test_requirement_6_1_6_2_performance_optimization():
    ...
    try:
        ...
        assert max_creation_time < 100, f"Session creation took {max_creation_time:.2f}ms, exceeds 100ms requirement"
        ...
        test_session_id = await session_manager.create_session(
            VideoGenerationRequest(prompt="Query test session for performance monitoring", duration_preference=30),
            "query_user"
        )
        ...
        assert max_query_time < 50, f"Status query took {max_query_time:.2f}ms, exceeds 50ms requirement"
...
async def test_requirement_8_1_8_2_monitoring_observability():
    ...
    try:
        ...
        log_test_session = await session_manager.create_session(
            VideoGenerationRequest(prompt="Log test session for monitoring", duration_preference=30),
            "log_user"
        )
        ...
        print("   ✅ Requirement 8.1: Session operations generate appropriate logs - PASSED")
        ...
        error_session = await session_manager.create_session(
            VideoGenerationRequest(prompt="Error test session for monitoring", duration_preference=30),
            "error_user"
        )
        
        # Simulate an error condition
        await session_manager.update_stage_and_progress(
            error_session, VideoGenerationStage.FAILED, 0.1, 
        ...
        assert error_state.error_message == "Detailed error message for testing requirement 8.2"
    ...
    try:
        ...
        for i in range(3):
            session_id = await session_manager.create_session(
                VideoGenerationRequest(prompt=f"Metrics test {i}", duration_preference=30),
                f"metrics_user_{i}"
            )
            ...
            if i == 0:
                ...
                await session_manager.update_stage_and_progress(
                    session_id, VideoGenerationStage.FAILED, 0.4, "Performance test failure"
                )
            # Leave one active
        ...
        for section in required_perf_sections:
            assert section in performance_data, f"Missing performance section: {section}"
            print(f"   ✅ {section}: Present")
        ...
        print("   ✅ Requirement 8.3: Performance metrics available for analysis - PASSED")
...
async def test_enhanced_get_statistics_implementation():
    ...
    try:
        ...
        for i in range(5):
            session_id = await session_manager.create_session(
                VideoGenerationRequest(prompt=f"Statistics test {i}", duration_preference=30),
        ...
        print(f"   ✅ Reliability metrics: Error rate {reliability['overall_error_rate']:.1%}")
...
async def main():
    ...
    try:
        ...
        if all(test_results):
            ...
            print("- ✅ Complete get_statistics() implementation with comprehensive metrics")
            print("- ✅ Session metrics collection and aggregation")
            print("- ✅ Health monitoring for session operations") 
            print("- ✅ Performance monitoring with thresholds and alerting")
            print("- ✅ Backward compatibility maintained")
            print("- ✅ All performance requirements met (< 100ms creation, < 50ms queries)")
