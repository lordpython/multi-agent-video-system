...
async def test_file_based_discovery():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        failed_session = next(
            s for s in legacy_sessions if s.session_id == "session_002"
        )
...
async def test_directory_based_discovery():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        session_files = {
            ...
            "request.json": {
                "prompt": "Directory-based session",
        ...
        assert target_session is not None, (
            f"Expected session not found. Found sessions: {[(s.session_id, s.progress) for s in legacy_sessions]}"
        )
        assert target_session.progress == 0.7, (
            f"Expected progress 0.7, got {target_session.progress}"
        )
...
async def test_migration_process():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        migration_manager.migration_completed_marker = (
            temp_path / ".migration_completed"
        )
        ...
        # Verify results
        assert results["migration_needed"]
        ...
        results["errors"] == [] and len(results) > 0
        ...
        assert not results2["migration_needed"]
        assert results2["already_completed"]
...
async def test_startup_migration_check():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert (
                results.get("migration_needed")
                or results.get("legacy_sessions_found", 0) > 0
            )
