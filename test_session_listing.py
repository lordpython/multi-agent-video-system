...
async def test_session_listing():
    ...
    try:
        ...
        assert len(user1_sessions) == 2, (
            f"Expected 2 sessions for user1, got {len(user1_sessions)}"
        )
        ...
        assert len(user2_sessions) == 1, (
            f"Expected 1 session for user2, got {len(user2_sessions)}"
        )
        ...
        assert len(completed_sessions) == 1, (
            f"Expected 1 completed session, got {len(completed_sessions)}"
        )
        ...
        assert len(failed_sessions) == 1, (
            f"Expected 1 failed session, got {len(failed_sessions)}"
        )
        ...
        assert len(processing_sessions) == 1, (
            f"Expected 1 processing session, got {len(processing_sessions)}"
        )
        ...
        assert len(sessions_page1) == 2, (
            f"Expected 2 sessions on page 1, got {len(sessions_page1)}"
        )
        assert pagination["total_count"] == 3, (
            f"Expected total count 3, got {pagination['total_count']}"
        )
        assert pagination["total_pages"] == 2, (
            f"Expected 2 total pages, got {pagination['total_pages']}"
        )
        assert pagination["has_next"], "Expected has_next=True"
        assert not pagination["has_prev"], "Expected has_prev=False"
        ...
        assert len(sessions_page2) == 1, (
            f"Expected 1 session on page 2, got {len(sessions_page2)}"
        )
        assert not pagination2["has_next"], "Expected has_next=False on page 2"
        assert pagination2["has_prev"], "Expected has_prev=True on page 2"
        ...
        assert len(user1_completed) == 1, (
            f"Expected 1 completed session for user1, got {len(user1_completed)}"
        )
        ...
        assert stats.total_sessions == 3, (
            f"Expected 3 total sessions in stats, got {stats.total_sessions}"
        )
        assert stats.completed_sessions == 1, (
            f"Expected 1 completed session in stats, got {stats.completed_sessions}"
        )
        assert stats.failed_sessions == 1, (
            f"Expected 1 failed session in stats, got {stats.failed_sessions}"
        )
        assert stats.active_sessions == 1, (
            f"Expected 1 active session in stats, got {stats.active_sessions}"
        )
        ...
        assert len(limited_sessions) == 2, (
            f"Expected 2 sessions with limit, got {len(limited_sessions)}"
        )
...
async def test_session_registry_management():
    ...
    try:
        ...
        assert len(updated_sessions) == 1, (
            f"Expected 1 session after update, got {len(updated_sessions)}"
        )
        ...
        assert len(final_sessions) == 0, (
            f"Expected 0 sessions after deletion, got {len(final_sessions)}"
        )
