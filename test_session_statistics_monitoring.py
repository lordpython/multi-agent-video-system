    ...
    try:
        ...
        for i in range(3):
            request = VideoGenerationRequest(
                prompt=f"Test completed video {i}",
        ...
        for i in range(2):
            ...
            # Simulate failure
            await session_manager.update_stage_and_progress(
                session_id, VideoGenerationStage.FAILED, 0.5, 
        ...
        for i in range(4):
            request = VideoGenerationRequest(
                prompt=f"Test active video {i}",
                duration_preference=30
            )
            session_id = await session_manager.create_session(request, f"user_active_{i}")
            test_sessions.append(session_id)
        ...
        assert session_counts.get("total") == 9, f"Expected 9 total sessions, got {session_counts.get('total')}"
        assert session_counts.get("completed") == 3, f"Expected 3 completed sessions, got {session_counts.get('completed')}"
        assert session_counts.get("failed") == 2, f"Expected 2 failed sessions, got {session_counts.get('failed')}"
        assert session_counts.get("active") == 4, f"Expected 4 active sessions, got {session_counts.get('active')}"
        ...
        assert completion_times.get("sample_size") == 3, "Should have 3 completed sessions for timing"
        ...
        assert abs(actual_error_rate - expected_error_rate) < 0.01, f"Expected error rate ~{expected_error_rate:.2f}, got {actual_error_rate:.2f}"
        
        expected_success_rate = 1.0 - expected_error_rate
        actual_success_rate = reliability_metrics.get("success_rate", 0)
        assert abs(actual_success_rate - expected_success_rate) < 0.01, f"Expected success rate ~{expected_success_rate:.2f}, got {actual_success_rate:.2f}"
        
        # Verify throughput metrics
        throughput_metrics = stats.get("throughput_metrics", {})
        assert throughput_metrics.get("sessions_last_hour") == 9, "All sessions should be within last hour"
        assert throughput_metrics.get("sessions_last_day") == 9, "All sessions should be within last day"
        ...
        print(f"   - Average completion time: {completion_times.get('average_seconds', 0):.2f}s")
...
async def test_monitoring_dashboard():
    ...
    try:
        ...
        for i in range(3):
            request = VideoGenerationRequest(
                prompt=f"Dashboard test video {i}",
                duration_preference=30
            )
...
async def test_backward_compatibility():
    ...
    try:
        ...
        for i in range(2):
            request = VideoGenerationRequest(
                prompt=f"Compatibility test video {i}",
                duration_preference=30
            )
            session_id = await session_manager.create_session(request, f"compat_user_{i}")
...
async def main():
    ...
    try:
        ...
        perf_metrics = await test_performance_metrics()
        dashboard_data = await test_monitoring_dashboard()
        health_status, health_check = await test_health_monitoring()
        session_metadata = await test_backward_compatibility()
        ...
        print(f"Success rate: {stats.get('reliability_metrics', {}).get('success_rate', 0):.1%}")
