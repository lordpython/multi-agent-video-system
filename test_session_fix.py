...
async def test_session_management():
    ...
    try:
        ...
        if (
            updated_state
            and updated_state.current_stage == VideoGenerationStage.RESEARCHING
        ):
...
async def test_video_generation_workflow():
    ...
    try:
        ...
        if result.get("success"):
            ...
            if state:
                print(
                    f"✅ Session state accessible through orchestration: {state.current_stage}"
                )
            else:
            ...
            print(
                f"❌ Video generation session creation failed: {result.get('error_message')}"
            )
            return False
...
async def main():
    ...
    print("📊 Test Results:")
    print(
        f"  Session Management: {'✅ PASSED' if session_test_passed else '❌ FAILED'}"
    )
    print(
        f"  Video Generation Workflow: {'✅ PASSED' if workflow_test_passed else '❌ FAILED'}"
    )
