...
class ADKServerTester:
        ...
        try:
            ...
            for i in range(max_wait):
                ...
                if self.web_process.poll() is not None:
                    ...
                    print(f"❌ Web server process exited early:")
        ...
        try:
            ...
            for i in range(max_wait):
                ...
                if self.api_process.poll() is not None:
                    ...
                    print(f"❌ API server process exited early:")
                    print(f"STDOUT: {stdout}")
                    print(f"STDERR: {stderr}")
                    return False
    ...
    def test_web_agent_discovery(self) -> bool:
        ...
        try:
            ...
            if isinstance(agents_data, list):
                ...
                discovered_agents = [agent.get('name', '') for agent in agents_data['agents']]
            else:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                session_response = requests.post(
                    f"{self.base_web_url}/api/sessions",
                    json=session_data,
                    timeout=10
                )
                
                if session_response.status_code not in [200, 201]:
                    print(f"❌ Failed to create session for {test_case['agent']}: {session_response.status_code}")
                    continue
                ...
                if message_response.status_code == 200:
                    ...
                    print(f"   ❌ {test_case['description']} failed: {message_response.status_code}")
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                if execution_response.status_code == 200:
                    ...
                    print(f"   ❌ {test_case['description']} failed: {execution_response.status_code}")
    ...
    def test_agent_accessibility(self) -> bool:
        ...
        for agent in self.expected_agents:
            try:
                response = requests.get(f"{self.base_web_url}/api/agents/{agent}", timeout=10)
                if response.status_code == 200:
                    print(f"   ✅ {agent} accessible via web interface")
                else:
                    print(f"   ❌ {agent} not accessible via web interface: {response.status_code}")
        ...
        for agent in self.expected_agents:
            try:
                response = requests.get(f"{self.base_api_url}/agents/{agent}", timeout=10)
                if response.status_code == 200:
                    print(f"   ✅ {agent} accessible via API server")
                else:
                    print(f"   ❌ {agent} not accessible via API server: {response.status_code}")
    ...
    def run_all_tests(self) -> bool:
        ...
        try:
            ...
            if (self.web_process and self.web_process.poll() is None and 
                self.api_process and self.api_process.poll() is None):
        ...
        print("\n" + "=" * 60)
        if all_passed:
            print("🎉 ALL TESTS PASSED! ADK web and API server integration working correctly.")
        else:
