...
def demo_retry_mechanisms():
    ...
    retry_config = RetryConfig(
        max_attempts=3, base_delay=0.5, max_delay=5.0, exponential_base=2.0, jitter=True
    )
    ...
    @retry_with_exponential_backoff(
        retry_config=retry_config, exceptions=(ProcessingError,)
    )
...
def demo_fallback_strategies():
    ...
    try:
        result = manager.execute_with_fallback(
            primary_service, [fallback_service_1, fallback_service_2]
        )
...
def demo_health_monitoring():
    ...
    monitor.service_registry.register_service(
        "healthy_service", health_check_func=healthy_service, critical=False
    )

    monitor.service_registry.register_service(
        "degraded_service", health_check_func=degraded_service, critical=True
    )

    monitor.service_registry.register_service(
        "failing_service", health_check_func=failing_service, critical=True
    )
