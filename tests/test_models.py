...
class TestVideoGenerationRequest:
    ...
    def test_duration_validation(self):
        ...
        with pytest.raises(ValidationError):
            VideoGenerationRequest(
                prompt="Valid prompt for testing", duration_preference=5
            )

        # Test too long duration
        with pytest.raises(ValidationError):
            VideoGenerationRequest(
                prompt="Valid prompt for testing", duration_preference=700
            )
...
class TestVideoScript:
    ...
    def test_scene_sequence_validation(self):
        ...
        with pytest.raises(ValidationError):
            VideoScript(title="Test Video", total_duration=60.0, scenes=scenes)
...
class TestAssetModels:
    ...
    def test_asset_collection(self):
        ...
        collection = AssetCollection(
            images=[asset], videos=[], metadata={"total_count": 1}
        )
...
class TestAssemblyModels:
    ...
    def test_assembly_request(self):
        ...
        audio_assets = AudioAssets(
            voice_files=["audio.wav"], timing_data={}, synchronization_markers=[]
        )
        ...
        script = VideoScript(title="Test Video", total_duration=30.0, scenes=[scene])
...
class TestHelperFunctions:
    ...
    def test_validate_video_script_consistency(self):
        ...
        script_valid = VideoScript(
            title="Valid Script", total_duration=60.0, scenes=scenes_valid
        )
        ...
        script_invalid_duration = VideoScript(
            title="Invalid Duration Script", total_duration=60.0, scenes=scenes_valid
        )
        ...
        script_invalid_sequence = VideoScript(
            title="Invalid Sequence Script", total_duration=60.0, scenes=scenes_valid
        )
