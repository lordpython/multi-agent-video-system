...
class TestGeminiTTSTool:
    ...
    def test_gemini_tts_success(self, mock_client_class, mock_gemini_tts_response):
        ...
        mock_client.models.generate_content_stream.return_value = (
            mock_gemini_tts_response
        )
        mock_client_class.return_value = mock_client

        result = generate_speech_with_gemini(
            "Hello world", voice_name="Zephyr", temperature=1.0
        )
    ...
    @patch.dict("os.environ", {"GEMINI_API_KEY": "test_api_key"})
    @patch("google.genai.Client")
    def test_gemini_tts_with_invalid_voice(
        self, mock_client_class, mock_gemini_tts_response
    ):
    ...
    def test_gemini_tts_request_exception(self, mock_client_class):
        ...
        assert result["audio_files"][0]["status"] == "error"
        assert (
            "Failed to generate speech with Gemini TTS"
            in result["audio_files"][0]["error"]
        )
...
class TestAudioProcessingTools:
    ...
    def test_convert_audio_format_success(self, mock_subprocess):
        ...
        with (
            ...
            patch("builtins.open", mock_open(read_data=b"converted_audio_data")),
        ):
    ...
    def test_convert_audio_format_no_ffmpeg(self, mock_subprocess):
        ...
        assert (
            result["converted_audio"] == original_data_base64
