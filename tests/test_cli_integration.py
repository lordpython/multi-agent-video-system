...
def mock_session_manager(temp_storage):
    ...
    from video_system.utils.session_manager import SessionManager

    with patch("video_system.api.cli.get_session_manager") as mock:
...
class TestCLICommands:
    ...
    def test_generate_command_basic(
        self, runner, mock_session_manager, mock_initialize_system
    ):
    ...
    def test_generate_command_with_all_options(
        ...
        result = runner.invoke(
            ...
            [
                "generate",
                "--prompt",
                "Create an educational video about machine learning",
                "--duration",
                "120",
                "--style",
                "educational",
    ...
    def test_wait_for_completion_success(
        ...
        with patch.object(
            ...
            with patch("time.sleep"):  # Speed up the test
                result = runner.invoke(
                    cli, ["generate", "--prompt", "Test video about AI", "--wait"]
                )
...
class TestCLIErrorHandling:
    ...
    def test_generate_command_system_error(self, runner, mock_initialize_system):
        ...
        result = runner.invoke(cli, ["generate", "--prompt", "Test video about AI"])
...
class TestCLIServeCommand:
    ...
    def test_serve_command_with_options(self, runner):
        ...
        with patch("uvicorn.run") as mock_run:
            result = runner.invoke(
                cli, ["serve", "--host", "0.0.0.0", "--port", "9000", "--reload"]
            )
    ...
    def test_serve_command_missing_dependencies(self, runner):
        ...
        with patch("video_system.cli.uvicorn", None):
            with patch(
                "builtins.__import__",
                side_effect=ImportError("No module named 'uvicorn'"),
            ):
                result = runner.invoke(cli, ["serve"])
