...
class TestImagenGenerationTool:
    ...
    def test_imagen_generation_without_api_key(self):
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            assert "GEMINI_API_KEY environment variable is not set" in result["images"][0]["error"]
    
    @patch.dict('os.environ', {'GEMINI_API_KEY': 'test_api_key'})
    @patch('google.genai.Client')
    @patch('PIL.Image.open')
    def test_imagen_generation_success(self, mock_pil_open, mock_client_class, mock_imagen_response):
        ...
        result = generate_imagen_image("mountain landscape", aspect_ratio="1:1", number_of_images=1)
    ...
    @patch('google.genai.Client')
    @patch('PIL.Image.open')
    def test_imagen_generation_with_invalid_parameters(self, mock_pil_open, mock_client_class, mock_imagen_response):
        ...
        result = generate_imagen_image("test", aspect_ratio="invalid_ratio", person_generation="invalid_policy")
...
class TestStableDiffusionTool:
    ...
    def test_stable_diffusion_without_api_key(self):
        ...
        with patch.dict('os.environ', {}, clear=True):
            ...
            assert len(result["images"]) == 1
            assert result["images"][0]["status"] == "error"
            assert "STABILITY_API_KEY environment variable is not set" in result["images"][0]["error"]
    
    @patch.dict('os.environ', {'STABILITY_API_KEY': 'test_api_key'})
    @patch('requests.post')
    def test_stable_diffusion_generation_success(self, mock_post, mock_stable_diffusion_response):
    ...
    @patch.dict('os.environ', {'STABILITY_API_KEY': 'test_api_key'})
    @patch('requests.post')
    def test_stable_diffusion_parameter_clamping(self, mock_post, mock_stable_diffusion_response):
...
class TestImageGenerationWorkflow:
    ...
    def test_multi_service_image_generation(self):
        ...
        imagen_optimization = optimize_image_prompt("A forest scene", target_service="imagen4")
        sd_optimization = optimize_image_prompt("A forest scene", target_service="stable_diffusion")
        ...
        assert imagen_optimization["recommended_params"] != sd_optimization["recommended_params"]
    ...
    def test_consistency_across_generations(self):
        ...
        scene1 = optimize_image_prompt("A park", consistency_elements=consistency_elements)
        scene2 = optimize_image_prompt("A playground", consistency_elements=consistency_elements)
    ...
    @patch('google.genai.Client')
    @patch('PIL.Image.open')
    def test_image_generation_agent_tool_integration(self, mock_pil_open, mock_client_class):
