    ...
    def test_unsplash_search_without_api_key(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            ...
            assert (
                ...
                in result["results"][0]["alt_description"]
            )
...
class TestPixabaySearchTool:
    ...
    def test_pixabay_search_without_api_key(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            ...
            assert (
                ...
                in result["results"][0]["tags"]
            )

    @patch.dict("os.environ", {"PIXABAY_API_KEY": "test_api_key"})
    @patch("requests.get")
    def test_pixabay_photos_search_success(
        self, mock_get, mock_pixabay_photos_response
    ):
        ...
        result = search_pixabay_media(
            "mountains", media_type="photo", category="nature"
        )
    ...
    @patch.dict("os.environ", {"PIXABAY_API_KEY": "test_api_key"})
    @patch("requests.get")
    def test_pixabay_videos_search_success(
        self, mock_get, mock_pixabay_videos_response
    ):
...
class TestAssetSourcingWorkflow:
    ...
    def test_asset_format_standardization(self):
        ...
        required_fields = ["id", "media_type", "source", "usage_rights"]
    ...
    def test_error_handling_scenarios(self):
        ...
        with patch.dict("os.environ", {}, clear=True):
            ...
            assert (
                "UNSPLASH_ACCESS_KEY"
                in unsplash_result["results"][0]["alt_description"]
            )
    ...
    def test_asset_sourcing_agent_tool_integration(self):
        ...
        with patch("requests.get") as mock_get:
            ...
            pexels_tool = next(
                tool for tool in tools if tool.__name__ == "search_pexels_media"
            )
    ...
    try:
        ...
        if pexels_result.get("total_results", 0) > 0:
            ...
            print(
                ...
                pexels_result.get("results", [{}])[0].get("alt", "Unknown error"),
            )
    ...
    try:
        ...
        if unsplash_result.get("total_results", 0) > 0:
            ...
            print(
                ...
                unsplash_result.get("results", [{}])[0].get(
                    "alt_description", "Unknown error"
                ),
            )
    except Exception as e:
    ...
    try:
        ...
        if pixabay_result.get("total_results", 0) > 0:
            ...
            print(
                ...
                pixabay_result.get("results", [{}])[0].get("tags", "Unknown error"),
            )
    except Exception as e:
