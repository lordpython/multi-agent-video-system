...
class TestSessionManager:
    ...
    def test_list_sessions(self, session_manager, sample_request):
        ...
        session_manager.create_session(sample_request, user_id="user1")
    ...
    def test_get_statistics(self, session_manager, sample_request):
        ...
        session_manager.update_session_status(
            session1, VideoStatus.PROCESSING, progress=0.5
        )
        session_manager.update_session_status(
            session2, VideoStatus.COMPLETED, progress=1.0
        )
        session_manager.update_session_status(
            session3, VideoStatus.PROCESSING, progress=0.3
        )
...
class TestProgressMonitor:
    ...
    def sample_request(self):
        """Create sample video generation request."""
        return VideoGenerationRequest(prompt="Test video", duration_preference=60)

    def test_start_session_monitoring(
        self, progress_monitor, session_manager, sample_request
    ):
    ...
    def test_update_stage_progress(
        ...
        success = progress_monitor.update_stage_progress(
            session_id=session_id, stage=SessionStage.RESEARCHING, progress=0.5
        )
        assert success is True
    ...
    def test_complete_session_with_failure(
        self, progress_monitor, session_manager, sample_request
    ):
        ...
        progress_monitor.update_stage_progress(
            session_id, SessionStage.RESEARCHING, 0.3
        )

        # Complete with failure
        error_message = "Test error"
        success = progress_monitor.complete_session(
            session_id=session_id, success=False, error_message=error_message
        )
        assert success is True
    ...
    def test_get_session_progress(
        ...
        progress_monitor.update_stage_progress(
            session_id, SessionStage.RESEARCHING, 0.5
        )
    ...
    def test_custom_stage_weights(
        ...
        success = progress_monitor.start_session_monitoring(
            session_id=session_id, stage_weights=custom_weights
        )
        assert success is True
...
class TestMaintenanceManager:
    ...
    def maintenance_manager(self, session_manager, temp_dir):
        ...
        return MaintenanceManager(session_manager=session_manager, temp_dir=temp_dir)
    ...
    def test_cleanup_expired_sessions(
        self, maintenance_manager, session_manager, sample_request
    ):
    ...
    def test_force_cleanup_session(
        self, maintenance_manager, session_manager, sample_request
    ):
...
class TestIntegration:
    ...
    def test_error_handling_integration(self, temp_storage, sample_request):
        ...
        progress_monitor.update_stage_progress(
            session_id, SessionStage.RESEARCHING, 0.8
        )
    ...
    def test_concurrent_sessions(self, temp_storage, sample_request):
        ...
        for i in range(3):
            session_id = session_manager.create_session(
                sample_request, user_id=f"user_{i}"
            )
