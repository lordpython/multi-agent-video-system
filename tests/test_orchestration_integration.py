...
class TestOrchestrationTools:
    ...
    def test_coordinate_research_tool(self):
        ...
        result = coordinate_research(
            topic="renewable energy", session_id=self.session_id
        )
    ...
    def test_coordinate_assets_tool(self):
        ...
        result = coordinate_assets(
            script=script.model_dump(), session_id=self.session_id
        )
    ...
    def test_error_handling(self):
        ...
        result = coordinate_research(
            topic="test topic", session_id="invalid-session-id"
        )
    ...
    def test_workflow_sequence(self):
        ...
        assets_result = coordinate_assets(
            script=story_result["script"], session_id=self.session_id
        )
...
class TestRootAgentIntegration:
    ...
    def test_execute_complete_workflow_tool(self):
        ...
        start_result = start_tool(prompt="Test video prompt", duration_preference=60)
        ...
        for tool in root_agent.tools:
            if (
                ...
                and tool.__name__ == "execute_complete_workflow"
            ):
                execute_tool = tool
                break
...
class TestErrorRecoveryAndRetry:
    ...
    def test_error_logging(self, mock_logger):
        ...
        # Create a session
        request = VideoGenerationRequest(prompt="Test prompt", duration_preference=60)
        session_state = create_session_state(request)
        session_id = session_state.session_id

        # Mock an exception during research
        with patch(
            "video_system.orchestration_tools.coordinate_research",
