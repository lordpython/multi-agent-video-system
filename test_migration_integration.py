...
async def test_migration_integration():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        try:
            ...
            assert migration_status["runtime_migration"]["completed"], (
            ...
            if migration_results.get("migration_needed"):
                assert migration_results["migrated_sessions"] == 2, (
                    f"Expected 2 migrated sessions, got {migration_results['migrated_sessions']}"
                )
                print(
                    f"✓ Successfully migrated {migration_results['migrated_sessions']} sessions"
                )
            else:
            ...
            for session_state in all_sessions:
                print(
                    f"Session {session_state.session_id}: {session_state.request.prompt[:50]}..."
                )
...
async def test_no_migration_needed():
    ...
    session_service = InMemorySessionService()
    session_manager = VideoSystemSessionManager(
        session_service=session_service, run_migration_check=True
    )
    ...
    assert migration_status["runtime_migration"]["completed"], (
        "Migration check should complete"
    )

    migration_results = migration_status["runtime_migration"]["results"]
    assert not migration_results.get("migration_needed", True), (
