...
def validate_staging_bucket():
    ...
    try:
        result = subprocess.run(
            ["gsutil", "ls", staging_bucket], capture_output=True, text=True, timeout=10
        )
        ...
        logger.warning(
            "Could not verify bucket accessibility (gsutil not available or timeout)"
        )
...
def validate_deployment_files():
    ...
    required_files = ["deploy.py", "grant_permissions.sh", "run.py"]
        ...
        if os.path.exists(file_path):
            ...
            if file_name.endswith(".sh") and os.name != "nt":
                if os.access(file_path, os.X_OK):
                    ...
                    logger.warning(
                        f"{file_name} is not executable, run: chmod +x {file_path}"
                    )
        else:
...
def main():
    ...
    if all(results):
        ...
        logger.error(
            f"✗ {failed_count} validation(s) failed. Please fix the issues above."
        )
        return 1
