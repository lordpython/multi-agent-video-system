...
class DockerDeployment:
    ...
    def build_image(
        self, tag: str = "multi-agent-video-system:latest", no_cache: bool = False
    ) -> bool:
        """Build Docker image."""
        try:
    ...
    def start_services(
        self, profiles: Optional[List[str]] = None, detached: bool = True
    ) -> bool:
    ...
    def _check_video_system_health(self) -> bool:
        ...
        try:
            cmd = [
                ...
                "python",
                "-c",
                "from video_system.shared_libraries.config_manager import validate_system_configuration; exit(0 if not validate_system_configuration() else 1)",
            ]
...
Examples:
  ...
  python docker_deploy.py --cleanup --volumes         # Clean up including volumes
    ...
    parser.add_argument(
        "--no-wait", action="store_true", help="Don't wait for services"
    )
    parser.add_argument(
        "--volumes", action="store_true", help="Remove volumes during cleanup"
    )
    parser.add_argument(
        "--images", action="store_true", help="Remove images during cleanup"
    )
    ...
    try:
        if args.deploy:
            success = deployment.deploy(
                profiles=args.profiles, build=not args.no_build, wait=not args.no_wait
            )
        else:
            if args.build:
            ...
            if args.logs:
                success &= deployment.show_logs(
                    service=args.logs, follow=args.follow, tail=args.tail
                )

            if args.cleanup:
                success &= deployment.cleanup(volumes=args.volumes, images=args.images)
