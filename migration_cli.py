    ...
    async def _status():
        try:
            ...
            if migration_status.get("runtime_migration", {}).get("completed"):
                ...
                console.print("[yellow]⏳[/yellow] Migration check in progress or not started")
            
            # Display migration results
            runtime_results = migration_status.get("runtime_migration", {}).get("results")
            if runtime_results:
            ...
            if migration_status.get("last_migration"):
                ...
                info_table.add_row("Started", last_migration.get("migration_started", "N/A"))
                info_table.add_row("Completed", last_migration.get("migration_completed", "N/A"))
                info_table.add_row("Total Sessions", str(last_migration.get("total_sessions", 0)))
...
def remigrate(confirm):
    ...
    if not confirm:
        console.print("[yellow]This will force remigration of all legacy sessions.[/yellow]")
        ...
        try:
            ...
            if legacy_sessions:
                ...
                if len(legacy_sessions) > 10:
                    console.print(f"[dim]... and {len(legacy_sessions) - 10} more sessions[/dim]")
            else:
