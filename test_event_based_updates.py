...
async def test_event_based_state_updates():
    ...
    try:
        ...
        await session_manager.update_session_state(
            session_id, test_field="test_value", progress=0.5
        )
        ...
        if new_event_count > initial_event_count:
            logger.info(
                "✅ State update created new event - using append_event correctly"
            )
            ...
            if latest_event.actions and latest_event.actions.state_delta:
                logger.info(
                    f"✅ Event has state_delta: {list(latest_event.actions.state_delta.keys())}"
                )
            else:
            ...
            logger.error(
                "❌ State update did not create new event - not using append_event"
            )
            return False
        ...
        if final_event_count > initial_event_count:
            logger.info(
                "✅ Stage update created new event - using append_event correctly"
            )
            ...
            logger.error(
                "❌ Stage update did not create new event - not using append_event"
            )
            return False
        ...
        for i, event in enumerate(updated_session.events):
            logger.info(
                f"Event {i}: author='{event.author}', timestamp={event.timestamp}"
            )

        # Clean up
        await session_manager.delete_session(session_id)
        logger.info(
            "✅ Test completed successfully - all state updates use proper event-based mechanism"
        )
...
async def main():
    ...
    if success:
        logger.info(
            "🎉 ALL TESTS PASSED - Event-based state updates working correctly!"
        )
        return 0
    else:
        logger.error(
            "💥 TESTS FAILED - Event-based state updates not working properly!"
        )
        return 1
