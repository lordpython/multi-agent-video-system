...
class VisualDescriptionInput(BaseModel):
    ...
    scene_content: str = Field(
        description="Content of the scene to generate visuals for"
    )
...
class VisualEnhancementInput(BaseModel):
    ...
    existing_requirements: List[str] = Field(
        description="Existing visual requirements to enhance"
    )
...
def generate_visual_descriptions(
    scene_content: str, style: str = "professional", duration: float = 10.0
) -> Dict[str, Any]:
    """
    ...
    try:
        if not scene_content.strip():
            return {"error": "Scene content cannot be empty", "visual_requirements": []}
...
def _determine_shot_type(content: str, shot_index: int, total_shots: int) -> str:
    ...
    if shot_index == 0:
        ...
        content_lower = content.lower()
        if any(
            word in content_lower for word in ["data", "chart", "graph", "statistic"]
        ):
            return "detail_shot"
...
def _create_visual_requirements(
    visual_elements: List[str], style_requirements: List[str], content: str
) -> List[str]:
    ...
    if any(
        word in content.lower() for word in ["important", "key", "crucial", "essential"]
    ):
...
def _create_timing_suggestions(
    requirements: List[str], duration: float
) -> List[Dict[str, Any]]:
...
def _get_audience_specific_requirements(audience: str) -> List[str]:
    ...
    audience_map = {
        ...
        "senior": ["Clear, high-contrast visuals", "Traditional, respectful imagery"],
    }
