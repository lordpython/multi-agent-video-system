...
class ScriptGenerationInput(BaseModel):
    ...
    research_data: Dict[str, Any] = Field(
        description="Research data containing facts, sources, and key points"
    )
    target_duration: int = Field(
        default=60, description="Target video duration in seconds"
    )
...
class SceneBreakdownInput(BaseModel):
    ...
    script_content: str = Field(
        description="Raw script content to break down into scenes"
    )
    target_duration: int = Field(
        default=60, description="Target total duration in seconds"
    )
    ...
    try:
        ...
        if not isinstance(research_data, dict):
            error = ValidationError(
                "Research data must be a dictionary", field="research_data"
            )
        ...
        if not (10 <= target_duration <= 600):
            error = ValidationError(
                "Target duration must be between 10 and 600 seconds",
        ...
        if style not in valid_styles:
            error = ValidationError(
                f"Style must be one of: {', '.join(valid_styles)}", field="style"
            )
            log_error(logger, error)
            return create_error_response(error)

        logger.info(
            f"Generating video script with duration: {target_duration}s, style: {style}, tone: {tone}"
        )
        ...
        if not facts and not key_points:
            error = ProcessingError(
                "Insufficient research data to generate script - no facts or key points provided"
            )
        ...
        if total_content_length < 50:
            error = ProcessingError(
                "Research content is too brief to generate a meaningful script"
            )
        ...
        if validation_errors:
            error = ProcessingError(
                f"Scene validation failed: {'; '.join(validation_errors)}"
            )
        ...
        logger.info(
            f"Successfully generated script with {len(scenes)} scenes for {target_duration}s video"
        )
        ...
        error = ProcessingError(
            f"Unexpected error during script generation: {str(e)}", original_exception=e
        )
...
def create_scene_breakdown(
    script_content: str, target_duration: int = 60, scene_count: int = 5
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if not isinstance(script_content, str):
            error = ValidationError(
                "Script content must be a string", field="script_content"
            )
            log_error(logger, error)
            return create_error_response(error)
        ...
        if not script_content.strip():
            error = ValidationError(
                "Script content cannot be empty", field="script_content"
            )
            log_error(logger, error)
            return create_error_response(error)
        ...
        if not (2 <= scene_count <= 10):
            error = ValidationError(
                "Scene count must be between 2 and 10", field="scene_count"
            )
            log_error(logger, error)
            return create_error_response(error)

        logger.info(
            f"Creating scene breakdown: {scene_count} scenes, {target_duration}s total"
        )

        # Validate content length
        if len(script_content.strip()) < 100:
            error = ProcessingError(
                "Script content is too short to create meaningful scenes"
            )
        ...
        if not segments or len(segments) < scene_count:
            error = ProcessingError(
                "Failed to split content into required number of segments"
            )
            log_error(
                logger,
                error,
                {"segments_created": len(segments), "required": scene_count},
            )
            return create_error_response(error)
        ...
        if not scenes:
            error = ProcessingError(
                "No valid scenes could be created from the script content"
            )
            log_error(logger, error)
            return create_error_response(error)
        ...
        if validation_errors:
            error = ProcessingError(
                f"Scene breakdown validation failed: {'; '.join(validation_errors)}"
            )
            log_error(logger, error, {"validation_errors": validation_errors})
            return create_error_response(error)
        ...
        log_error(
            logger, e, {"target_duration": target_duration, "scene_count": scene_count}
        )
        return create_error_response(e)

    except Exception as e:
        error = ProcessingError(
            f"Unexpected error during scene breakdown: {str(e)}", original_exception=e
        )
        log_error(
            logger,
            error,
            {"target_duration": target_duration, "scene_count": scene_count},
        )
        return create_error_response(error)


def _generate_title(
    key_points: List[str], facts: List[str], context: Dict[str, Any]
) -> str:
...
def _create_scenes_from_research(
    ...
    for i in range(scene_count):
        ...
        end_idx = (
            start_idx + content_per_scene if i < scene_count - 1 else len(all_content)
        )
...
def _create_scene_description(
    content: List[str], scene_num: int, total_scenes: int
) -> str:
...
def _generate_visual_requirements(content: List[str], style: str) -> List[str]:
    ...
    if style == "professional":
        ...
        requirements.extend(
            ["Engaging visuals", "Relevant imagery", "Clear presentation"]
        )
    ...
    for item in content[:2]:  # Use first 2 content items
        if any(keyword in item.lower() for keyword in ["data", "statistic", "number"]):
            ...
            keyword in item.lower() for keyword in ["technology", "tech", "digital"]
        ):
...
def _create_dialogue(
    content: List[str], tone: str, scene_num: int, total_scenes: int
) -> str:
...
def _split_content_into_segments(content: str, segment_count: int) -> List[str]:
    ...
    if len(paragraphs) >= segment_count:
        ...
        for i in range(segment_count):
            ...
            end_idx = (
                start_idx + sentences_per_segment
                if i < segment_count - 1
                else len(sentences)
            )
...
def _create_scene_from_segment(
    segment: str, scene_num: int, duration: float
) -> Dict[str, Any]:
...
def _validate_generated_scenes(
    scenes: List[Dict[str, Any]], target_duration: float
) -> List[str]:
    ...
    if abs(total_scene_duration - target_duration) > 2.0:  # Allow 2 second tolerance
        errors.append(
            f"Total scene duration ({total_scene_duration}s) doesn't match target ({target_duration}s)"
        )
...
def _validate_scene_breakdown(
    ...
    for i, scene in enumerate(scenes, 1):
        if scene.get("scene_number") != i:
            errors.append(
                f"Scene {i} has incorrect scene number: {scene.get('scene_number')}"
            )
    ...
    if abs(total_duration - target_duration) > 2.0:  # Allow 2 second tolerance
        errors.append(
            f"Total duration ({total_duration}s) doesn't match target ({target_duration}s)"
        )
