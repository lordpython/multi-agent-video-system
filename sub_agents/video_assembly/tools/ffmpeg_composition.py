...
class VideoCompositionRequest(BaseModel):
    ...
    video_assets: List[str] = Field(
        ..., description="List of video/image asset file paths"
    )
    audio_file: str = Field(..., description="Path to audio file")
    output_path: str = Field(..., description="Output video file path")
    scene_timings: List[Dict[str, Any]] = Field(
        ..., description="Scene timing information"
    )
    ...
    try:
        ...
        if not request.video_assets:
            ...
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
            )
        ...
        if not _check_ffmpeg_availability():
            error = ResourceError(
                "FFmpeg is not available on this system", resource_type="ffmpeg"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        logger.info(
            f"Starting video composition with {len(request.video_assets)} assets"
        )

        # Validate input files exist
        if not os.path.exists(request.audio_file):
            error = ValidationError(
                f"Audio file not found: {request.audio_file}", field="audio_file"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        missing_assets = [
            asset for asset in request.video_assets if not os.path.exists(asset)
        ]
        if missing_assets:
            error = ValidationError(
                f"Missing video assets: {missing_assets}", field="video_assets"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        validation_errors = _validate_media_files(
            request.video_assets, request.audio_file
        )
        if validation_errors:
            error = ValidationError(
                f"Media file validation failed: {'; '.join(validation_errors)}"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        if not _check_disk_space(request.output_path):
            error = ResourceError(
                "Insufficient disk space for video composition", resource_type="disk"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        try:
            ...
            if result.returncode != 0:
                ...
                log_error(
                    logger, error, {"command": " ".join(cmd[:5]) + "..."}
                )  # Log first 5 args only
                return VideoCompositionResponse(
                    success=False,
                    output_file="",
                    duration=0.0,
            ...
            error = TimeoutError(
                "FFmpeg command timed out after 10 minutes", timeout_duration=600.0
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        if not os.path.exists(request.output_path):
            error = ProcessingError(
                "Output video file was not created", stage="output_verification"
            )
            log_error(logger, error)
            return VideoCompositionResponse(
                success=False, output_file="", duration=0.0, error_message=error.message
        ...
        logger.info(
            f"Video composition successful: {request.output_path} ({duration:.2f}s)"
        )
        ...
        return VideoCompositionResponse(
            success=False, output_file="", duration=0.0, error_message=e.message
        )

    except Exception as e:
        error = ProcessingError(
            f"Unexpected error during video composition: {str(e)}", original_exception=e
        )
...
def _build_filter_complex(request: VideoCompositionRequest) -> str:
    ...
    for i, timing in enumerate(request.scene_timings):
        ...
        if i < len(request.video_assets):
            ...
            if _is_image_file(request.video_assets[i]):
                filters.append(
                    f"[v{i}]loop=loop=-1:size={request.fps * int(duration)}:start=0[loop{i}]"
                )
    ...
    if concat_inputs:
        concat_filter = (
            "".join(concat_inputs) + f"concat=n={len(concat_inputs)}:v=1:a=0[v]"
        )
