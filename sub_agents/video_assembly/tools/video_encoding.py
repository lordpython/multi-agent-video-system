...
class EncodingRequest(BaseModel):
    ...
    format: Optional[str] = Field("mp4", description="Output video format (mp4, webm, avi)")
    quality: Optional[str] = Field("high", description="Encoding quality (low, medium, high, ultra)")
    ...
    optimize_for: Optional[str] = Field("quality", description="Optimization target: 'quality', 'size', 'streaming'")
...
class EncodingResponse(BaseModel):
    ...
    video_info: Dict[str, Any] = Field(default={}, description="Video information after encoding")
...
def encode_video(request: EncodingRequest) -> EncodingResponse:
    ...
    try:
        ...
        logger.info(f"Size reduction: {original_size} -> {encoded_size} bytes (ratio: {compression_ratio:.2f})")
...
def _build_encoding_command(request: EncodingRequest, original_info: Dict[str, Any]) -> List[str]:
...
def _get_video_encoding_settings(request: EncodingRequest, original_info: Dict[str, Any]) -> List[str]:
...
def get_recommended_settings(file_size_mb: float, target_use: str) -> Dict[str, Any]:
    ...
    if target_use == "web":
        if file_size_mb > 100:
            return {"quality": "medium", "optimize_for": "size", "resolution": "1280x720"}
        else:
            return {"quality": "high", "optimize_for": "quality"}
    elif target_use == "mobile":
        return {"quality": "medium", "optimize_for": "size", "resolution": "854x480", "fps": 24}
