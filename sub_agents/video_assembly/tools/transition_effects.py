...
class TransitionRequest(BaseModel):
    ...
    input_segments: List[str] = Field(..., description="List of video segment file paths")
    transition_types: List[str] = Field(..., description="List of transition types for each segment")
    transition_durations: List[float] = Field(..., description="Duration of each transition in seconds")
...
class TransitionResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether transitions were applied successfully")
    output_file: str = Field(..., description="Path to video with transitions")
    total_duration: float = Field(..., description="Total duration of video with transitions")
    transitions_applied: List[str] = Field(default=[], description="List of transitions that were applied")
...
def _build_transition_filter_complex(request: TransitionRequest) -> str:
    ...
    for i in range(1, len(request.input_segments)):
        transition_type = request.transition_types[i-1] if i-1 < len(request.transition_types) else "crossfade"
        transition_duration = request.transition_durations[i-1] if i-1 < len(request.transition_durations) else 1.0
...
def _get_transition_filter(input1: str, input2: str, transition_type: str, duration: float) -> str:
