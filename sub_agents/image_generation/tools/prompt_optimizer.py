...
class PromptOptimizerInput(BaseModel):
    ...
    video_style: str = Field(
        ...
        description="Overall video style (professional, cinematic, documentary, etc.)",
    )
    consistency_elements: List[str] = Field(
        default=[], description="Elements to maintain consistency across images"
    )
    target_service: str = Field(
        default="imagen4",
        description="Target image generation service (imagen4, stable_diffusion)",
    )
...
def optimize_image_prompt(
    ...
    try:
        ...
        style_config = style_mappings.get(
            video_style.lower(), style_mappings["professional"]
        )
        ...
        if consistency_elements:
            ...
            prompt_parts.append(
                f"maintaining visual consistency with: {consistency_str}"
            )
        ...
        if target_service.lower() == "imagen4":
            ...
            recommended_params = {
                ...
                "quality": "hd",
                "style": "vivid"
                if video_style in ["cinematic", "artistic"]
                else "natural",
            }
        else:
            recommended_params = {}
...
def generate_style_variations(
    base_prompt: str, num_variations: int = 3
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            "base_prompt": base_prompt,
            "variations": [
                {"prompt": base_prompt, "style_modifier": "none", "variation_id": 1}
            ],
            "total_variations": 1,
