...
class PexelsSearchInput(BaseModel):
    ...
    media_type: str = Field(
        default="photos", description="Type of media: 'photos' or 'videos'"
    )
    orientation: str = Field(
        default="all",
        description="Image orientation: 'all', 'landscape', 'portrait', 'square'",
    )
...
@handle_api_errors
def _perform_pexels_search(
    query: str, per_page: int, media_type: str, orientation: str, api_key: str
) -> Dict[str, Any]:
    ...
    try:
        ...
        if response.status_code == 429:
            ...
            raise APIError(
                "Pexels API access forbidden", api_name="Pexels", status_code=403
            )
        ...
        data = response.json()
        logger.info(
            f"Successfully retrieved {len(data.get('photos' if media_type == 'photos' else 'videos', []))} results from Pexels"
        )
        return data

    except requests.exceptions.Timeout as e:
        raise TimeoutError(
            f"Pexels API request timed out: {str(e)}", timeout_duration=30.0
        )
...
def search_pexels_media(
    query: str, per_page: int = 15, media_type: str = "photos", orientation: str = "all"
) -> Dict[str, Any]:
    """
    ...
    if media_type not in ["photos", "videos"]:
        error = ValidationError(
            "media_type must be 'photos' or 'videos'", field="media_type"
        )
        log_error(logger, error)
        return create_error_response(error)
    ...
    if orientation not in ["all", "landscape", "portrait", "square"]:
        error = ValidationError(
            "orientation must be 'all', 'landscape', 'portrait', or 'square'",
            field="orientation",
        )
        log_error(logger, error)
        return create_error_response(error)
    ...
    if not api_key:
        error = APIError(
            "PEXELS_API_KEY environment variable is not set", api_name="Pexels"
        )
        log_error(logger, error)
        return create_error_response(error)
    ...
    try:
        ...
        raw_data = _perform_pexels_search(
            query.strip(), per_page, media_type, orientation, api_key
        )

        # Format the results
        formatted_results = _format_pexels_results(
            raw_data, query.strip(), per_page, media_type
        )

        logger.info(
            f"Pexels search completed successfully for query: '{query}' - {len(formatted_results['results'])} results"
        )
        ...
        log_error(
            logger, e, {"query": query, "per_page": per_page, "media_type": media_type}
        )
        ...
        error = APIError(
            f"Unexpected error during Pexels search: {str(e)}", api_name="Pexels"
        )
        log_error(
            logger,
            error,
            {"query": query, "per_page": per_page, "media_type": media_type},
        )
        return create_error_response(error)


def _format_pexels_results(
    data: Dict[str, Any], query: str, per_page: int, media_type: str
) -> Dict[str, Any]:
    ...
    for item in media_items:
        if media_type == "videos":
            ...
            formatted_result = {
                ...
                "alt": item.get(
                    "alt", f"Photo by {item.get('photographer', 'Unknown')} from Pexels"
                ),
