...
def test_basic_session_functionality():
    ...
    try:
        ...
        print(f"Session status: {session.status}, stage: {session.stage}, progress: {session.progress}")
        ...
        # Get updated session
        session = session_manager.get_session(session_id)
        print(f"Updated session status: {session.status}, stage: {session.stage}, progress: {session.progress}")
        ...
        stages = [SessionStage.SCRIPTING, SessionStage.ASSET_SOURCING, SessionStage.AUDIO_GENERATION]
        for stage in stages:
        ...
        print(f"System health - Sessions: {health.total_sessions}, Disk usage: {health.disk_usage_percent:.1f}%")
