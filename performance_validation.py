...
class PerformanceValidator:
    ...
    async def _test_basic_functionality(self):
        ...
        test_results = {"start_time": datetime.utcnow().isoformat(), "tests": {}}
        ...
        try:
            ...
            request_id = self.concurrent_processor.submit_request(
                request, user_id="validation_user", priority=RequestPriority.HIGH
            )
        ...
        logger.info(
            f"Basic functionality test completed: {'PASSED' if test_results['overall_passed'] else 'FAILED'}"
        )
    ...
    async def _test_concurrent_processing(self):
        ...
        try:
            ...
            low_id = self.concurrent_processor.submit_request(
                VideoGenerationRequest(
                    prompt="Low priority request", duration_preference=30
                ),
        ...
        logger.info(
            f"Concurrent processing test completed: {'PASSED' if test_results['overall_passed'] else 'FAILED'}"
        )
    ...
    async def _test_resource_management(self):
        ...
        logger.info(
            f"Resource management test completed: {'PASSED' if test_results['overall_passed'] else 'FAILED'}"
        )
    ...
    async def _test_rate_limiting(self):
        ...
        try:
            ...
            allowed, delay = self.rate_limiter.check_rate_limit(
                service_name, user_id="test_user"
            )
        ...
        try:
            ...
            test_results["tests"]["service_status"] = {
                ...
                "serper_allowed_rps": serper_status.allowed_rps
                if serper_status
                else None,
            ...
            test_results["tests"]["service_status"] = {"error": str(e), "passed": False}
        ...
        logger.info(
            f"Rate limiting test completed: {'PASSED' if test_results['overall_passed'] else 'FAILED'}"
        )
    ...
    async def _test_load_scenarios(self):
        ...
        test_results = {"start_time": datetime.utcnow().isoformat(), "load_tests": {}}
        ...
        logger.info(
            f"Load scenarios test completed: {'PASSED' if test_results['overall_passed'] else 'FAILED'}"
        )
    ...
    def _generate_report(self):
        ...
        if "system_state" in summary and "error" not in summary["system_state"]:
            ...
            print(
                f"  Processor Uptime: {summary['system_state']['processor_uptime']:.1f}s"
            )
            print(
                f"  Requests Processed: {summary['system_state']['total_requests_processed']}"
            )
