...
from video_system.orchestration_tools import (
    create_session_state, coordinate_research, get_session_state
)
...
async def test_orchestration_event_updates():
    ...
    try:
        ...
        if final_event_count > initial_event_count:
            logger.info("✅ Orchestration tools created new events - using append_event correctly")
            
            # Check the latest events for proper state_delta
            new_events = updated_session.events[initial_event_count:]
            for i, event in enumerate(new_events):
                if event.actions and event.actions.state_delta:
                    logger.info(f"✅ Event {i} has state_delta: {list(event.actions.state_delta.keys())}")
                else:
                    logger.warning(f"⚠️ Event {i} missing state_delta")
        else:
            logger.error("❌ Orchestration tools did not create new events - not using append_event")
            return False
        
        # Verify the session state was updated correctly
        final_state = await get_session_state(session_id)
        if final_state and hasattr(final_state, 'research_data') and final_state.research_data:
