...
class VirtualUser:
    ...
    async def _send_request(self) -> RequestResult:
        ...
        result = RequestResult(
            request_id=request_id, user_id=self.user_id, start_time=start_time
        )
        ...
        try:
            ...
            submitted_request_id = self.processor.submit_request(
                video_request, user_id=self.user_id, priority=RequestPriority.NORMAL
            )
    ...
    def _update_metrics(self, result: RequestResult):
        ...
        if result.end_time:
            ...
            self.metrics.min_response_time_ms = min(
                self.metrics.min_response_time_ms, result.response_time_ms
            )
            self.metrics.max_response_time_ms = max(
                self.metrics.max_response_time_ms, result.response_time_ms
            )
...
class LoadTester:
    ...
    async def run_load_test(
        self,
        config: LoadTestConfig,
        request_generator: Optional[Callable[[], VideoGenerationRequest]] = None,
    ) -> LoadTestMetrics:
    ...
    async def _execute_load_test(
        ...
        if config.test_type == LoadTestType.CONSTANT_LOAD:
            await self._run_constant_load(
                config, request_generator, metrics, stop_event
            )
    ...
    async def _monitor_resources(self, metrics: LoadTestMetrics, duration_seconds: int):
        ...
        while time.time() - start_time < duration_seconds:
            try:
                ...
                # Update peak values
                metrics.peak_cpu_percent = max(
                    metrics.peak_cpu_percent, resource_usage.cpu_percent
                )
                metrics.peak_memory_percent = max(
                    metrics.peak_memory_percent, resource_usage.memory_percent
                )
                metrics.peak_disk_percent = max(
                    metrics.peak_disk_percent, resource_usage.disk_percent
                )
