...
async def test_start_video_generation():
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.error(
                f"start_video_generation failed: {result.get('error_message')}"
            )
            return None
...
async def test_execute_complete_workflow(session_id: str):
    ...
    try:
        ...
        if result.get("success"):
            ...
            logger.error(
                f"execute_complete_workflow failed: {result.get('error_message')}"
            )
            return False
...
async def main():
    ...
    logger.info(
        f"  execute_complete_workflow: {'PASSED' if workflow_success else 'FAILED'}"
    )
    logger.info(
