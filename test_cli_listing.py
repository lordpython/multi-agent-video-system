...
async def setup_test_sessions():
    ...
    await session_manager.update_stage_and_progress(
        session_id1, VideoGenerationStage.COMPLETED, 1.0
    )
    ...
    await session_manager.update_stage_and_progress(
        session_id2, VideoGenerationStage.RESEARCHING, 0.3
    )
    ...
    await session_manager.update_stage_and_progress(
        session_id3, VideoGenerationStage.FAILED, 0.5, "Test error"
    )
...
async def main():
    ...
    try:
        ...
        success, stdout, stderr = test_cli_command(
            "python -m video_system.cli list --user test_user"
        )
        if success:
        ...
        success, stdout, stderr = test_cli_command(
            "python -m video_system.cli list --status-filter completed"
        )
        if success:
        ...
        success, stdout, stderr = test_cli_command(
            "python -m video_system.cli list --page 1 --page-size 2"
        )
        if success:
        ...
        success, stdout, stderr = test_cli_command(
            "python -m video_system.cli status --user test_user --limit 5"
        )
        if success:
