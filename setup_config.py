...
class ConfigurationSetup:
    ...
    def check_prerequisites(self) -> List[str]:
        ...
        if not mongo_found:
            issues.append(
                "MongoDB not found. Consider installing MongoDB for session management."
            )
        ...
        if not docker_found:
            issues.append(
                "Docker not found. Docker is required for containerized deployment."
            )
    ...
    def create_env_file(self, interactive: bool = True) -> bool:
        ...
        if interactive:
            ...
            print(
                "  - Run this script again with --validate to check your configuration"
            )
    ...
    def _print_config_summary(self, summary: Dict):
        ...
        print(
            f"    Staging Bucket: {'Configured' if gcp['has_staging_bucket'] else 'Not configured'}"
        )
        ...
        print(
            f"    Rate Limiting: {'Enabled' if perf['rate_limiting_enabled'] else 'Disabled'}"
        )
...
Examples:
  ...
  python setup_config.py --create-dirs     # Create necessary directories
    ...
    parser.add_argument(
        "--wizard", action="store_true", help="Run interactive setup wizard"
    )

    parser.add_argument(
        "--create-env", action="store_true", help="Create .env file from .env.example"
    )
