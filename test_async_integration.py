...
from video_system.orchestration_tools import (
    coordinate_research, coordinate_story, coordinate_assets, 
    coordinate_audio, coordinate_assembly, get_session_status
)
...
async def test_complete_workflow():
    ...
    try:
        ...
        if not result.get('success'):
            logger.error(f"Failed to start video generation: {result.get('error_message')}")
            return False
        ...
        workflow_result = await execute_complete_workflow(session_id, user_id="integration_test")
        
        if not workflow_result.get('success'):
            logger.error(f"Failed to execute workflow: {workflow_result.get('error_message')}")
            return False
        ...
        # Story phase
        story_result = await coordinate_story(research_result['research_data'], session_id, 90)
