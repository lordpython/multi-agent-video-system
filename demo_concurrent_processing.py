...
async def demo_concurrent_processing():
    ...
    try:
        ...
        requests = [
            (
                "Create a video about AI and machine learning",
                RequestPriority.HIGH,
                "ai_user",
            ),
            ("Make a cooking tutorial video", RequestPriority.NORMAL, "cooking_user"),
            (
                "Generate a travel vlog about Paris",
        ...
        for prompt, priority, user_id in requests:
            ...
            request_id = processor.submit_request(
                video_request, user_id=user_id, priority=priority
            )
        ...
        print(
            f"  Average Processing Time: {final_metrics.average_processing_time:.2f}s"
        )
...
async def demo_resource_management():
    ...
    try:
        ...
        print(
            f"  Memory: {usage.memory_percent:.1f}% ({usage.memory_available_gb:.1f} GB available)"
        )
        print(f"  Disk: {usage.disk_percent:.1f}% ({usage.disk_free_gb:.1f} GB free)")
        print(
            f"  Network: {usage.network_sent_mbps:.2f} Mbps sent, {usage.network_recv_mbps:.2f} Mbps received"
        )
        ...
        print(
            f"  CPU Cores: {availability['cpu']['available_cores']:.1f} / {availability['cpu']['total_cores']}"
        )
        ...
        print(
            f"  CPU Cores after allocation: {availability_after['cpu']['available_cores']:.1f}"
        )
        ...
        for i in range(5):
            ...
            print(
                f"  Sample {i + 1}: CPU {current_usage.cpu_percent:.1f}%, Memory {current_usage.memory_percent:.1f}%"
            )
...
async def demo_rate_limiting():
    ...
    for service in services_to_test:
        ...
        for i in range(5):
            ...
            print(
                f"    Request {i + 1}: {'✓ Allowed' if allowed else '✗ Rate Limited'} (delay: {delay:.2f}s)"
            )
...
async def demo_load_testing():
    ...
    try:
        ...
        for user_id, user_metrics in metrics.user_metrics.items():
            avg_response = user_metrics.total_response_time_ms / max(
                1, user_metrics.requests_completed
            )
            print(f"  {user_id}:")
