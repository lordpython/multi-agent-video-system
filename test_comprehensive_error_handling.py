...
async def test_session_creation_with_retry():
    ...
    session_manager = VideoSystemSessionManager(
        session_service=failing_service, enable_fallback=False
    )

    try:
        # This should fail after retries
        await session_manager.create_session("Test prompt")
...
async def test_fallback_mechanism():
    ...
    session_manager = VideoSystemSessionManager(
        session_service=failing_service, enable_fallback=True
    )
...
async def test_health_monitoring():
    ...
    try:
        ...
        logger.info(
            f"Health status retrieved: {health_status.get('healthy', 'unknown')}"
        )

        # Force health check
        health_check = await session_manager.force_health_check()
        logger.info(
            f"Health check completed: {health_check.get('overall_healthy', 'unknown')}"
        )

        # Check monitor metrics
        monitor_status = health_monitor.get_health_status()
        logger.info(
            f"Monitor metrics: {monitor_status['metrics']['operations_total']} total operations"
        )
...
async def test_graceful_degradation():
    ...
    try:
        ...
        try:
            await session_manager.create_session("Degradation test")
