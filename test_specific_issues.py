...
async def test_orchestrator_issue():
    ...
    print(
        f"📊 Orchestrator has sub_agents: {hasattr(video_orchestrator, 'sub_agents')}"
    )
    ...
    # Create runner
    runner = Runner(
        agent=video_orchestrator, app_name=app_name, session_service=session_service
    )
...
async def test_error_handling_issue():
    ...
    try:
        invalid_session = await session_service.get_session(
            app_name=app_name, user_id=user_id, session_id="non-existent-session"
        )
...
async def main():
    ...
    print("📊 SUMMARY:")
    print(
        f"  - Orchestrator coordination: {'✅ OK' if orchestrator_ok else '❌ FAILED'}"
    )
