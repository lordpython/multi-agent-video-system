...
def test_adk_command(agent_path: str, agent_name: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if (
                f"Running agent {agent_name}" in result.stdout
                or "Running agent" in result.stdout
            ):
                return (
                    True,
                    f"✅ {agent_name}: Successfully started and discovered by ADK",
                )
            else:
                return (
                    False,
                    f"❌ {agent_name}: Started but agent name not found in output",
                )
        else:
            ...
            if "Running agent" in result.stdout:
                return (
                    True,
                    f"✅ {agent_name}: Successfully started (exit code {result.returncode})",
                )
            else:
                return (
                    False,
                    f"❌ {agent_name}: Failed to start - {result.stderr[:200]}",
                )
...
def main():
    ...
    if successful_tests == total_tests:
        print(
            "🎉 All ADK commands work correctly! The canonical structure is properly implemented."
        )
        return 0
    else:
