...
class AdaptiveRateLimiter:
    ...
    def get_current_limits(self) -> RateLimitConfig:
        ...
        adjusted_config = RateLimitConfig(
            ...
            * self.current_multiplier,
            requests_per_hour=self.base_config.requests_per_hour
            * self.current_multiplier,
            burst_size=max(
                1, int(self.base_config.burst_size * self.current_multiplier)
            ),
    ...
    def _adjust_limits(self):
        ...
        with self.lock:
            ...
            health_score = self._calculate_health_score(
                avg_response_time, current_success_rate
            )
            ...
            logger.debug(
                f"Adaptive rate limiter adjusted: health={health_score:.2f}, multiplier={self.current_multiplier:.2f}"
            )
...
class RateLimiter:
    ...
    def add_service_limits(self, service_limits: ServiceLimits):
        ...
        with self.lock:
            ...
            if config.strategy in [
                ...
                self.sliding_windows[service_name] = SlidingWindowCounter(
                    60.0
                )  # 1 minute window
    ...
    def check_rate_limit(
        self, service_name: str, user_id: Optional[str] = None, tokens: float = 1.0
    ) -> Tuple[bool, float]:
        ...
        with self.lock:
            ...
            service_allowed, service_delay = self._check_service_limit(
                service_name, tokens
            )
    ...
    def record_request(
        ...
        with self.lock:
            ...
            if service_name in self.adaptive_limiters:
                self.adaptive_limiters[service_name].record_request(
    ...
    def get_service_status(self, service_name: str) -> Optional[RateLimitStatus]:
        ...
        with self.lock:
            ...
            if service_name in self.adaptive_limiters:
                current_config = self.adaptive_limiters[
                    service_name
                ].get_current_limits()
            else:
            ...
            if service_name in self.adaptive_limiters:
                adaptive_multiplier = self.adaptive_limiters[
                    service_name
                ].current_multiplier
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            recent_requests = [
                r for r in self.request_history if r.timestamp >= last_hour
            ]
            ...
            for service_name in self.service_limits.keys():
                service_requests = [
                    r for r in recent_requests if r.service_name == service_name
                ]
                service_stats[service_name] = {
                    "total_requests": len(service_requests),
                    "rate_limited": sum(1 for r in service_requests if r.rate_limited),
                    "success_rate": sum(1 for r in service_requests if r.success)
                    / max(1, len(service_requests)),
                    "avg_response_time": sum(
                        r.response_time_ms for r in service_requests
                    )
                    / max(1, len(service_requests)),
                }
            ...
            return {
                ...
                "rate_limited_percentage": (
                    rate_limited_requests / max(1, total_requests)
                )
                * 100,
    ...
    def _check_service_limit(
        self, service_name: str, tokens: float
    ) -> Tuple[bool, float]:
        ...
        elif current_config.strategy in [
            RateLimitStrategy.SLIDING_WINDOW,
            RateLimitStrategy.ADAPTIVE,
        ]:
            if service_name in self.sliding_windows:
                ...
                if current_rate < current_config.requests_per_second:
                    ...
                    delay = (
                        current_rate - current_config.requests_per_second
                    ) / current_config.requests_per_second
                    return False, min(delay, current_config.max_delay_seconds)

        return True, 0.0

    def _check_user_limit(
        self, user_id: Optional[str], tokens: float
    ) -> Tuple[bool, float]:
        ...
        if current_rate < self.user_limits.requests_per_second:
            ...
            delay = (
                current_rate - self.user_limits.requests_per_second
            ) / self.user_limits.requests_per_second
