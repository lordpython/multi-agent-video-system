...
class GoogleCloudConfig(BaseModel):
    ...
    use_vertexai: bool = Field(
        default=True, description="Use Vertex AI instead of ML Dev"
    )
    ...
    api_key: Optional[str] = Field(
        default=None, description="Google API Key for ML Dev"
    )
    credentials_path: Optional[str] = Field(
        default=None, description="Path to service account credentials"
    )
    staging_bucket: Optional[str] = Field(
        default=None, description="Staging bucket for deployment"
    )
...
class ExternalAPIConfig(BaseModel):
    ...
    unsplash_access_key: Optional[str] = Field(
        default=None, description="Unsplash access key"
    )
    ...
    stability_api_key: Optional[str] = Field(
        default=None, description="Stability AI API key"
    )
    gemini_api_key: Optional[str] = Field(
        default=None, description="Gemini API key for TTS"
    )
...
class DatabaseConfig(BaseModel):
    ...
    enable_session_encryption: bool = Field(
        default=True, description="Enable session encryption"
    )
...
class StorageConfig(BaseModel):
    ...
    video_output_dir: Path = Field(
        default=Path("./output"), description="Video output directory"
    )
    temp_dir: Path = Field(
        default=Path("./temp"), description="Temporary files directory"
    )
    asset_cache_dir: Path = Field(
        default=Path("./cache/assets"), description="Asset cache directory"
    )
    session_data_dir: Path = Field(
        default=Path("./data/sessions"), description="Session data directory"
    )
...
class LoggingConfig(BaseModel):
    ...
    max_log_file_size_mb: int = Field(
        default=10, description="Maximum log file size in MB"
    )
...
class PerformanceConfig(BaseModel):
    ...
    default_requests_per_second: float = Field(
        default=10.0, description="Default requests per second"
    )
    default_requests_per_minute: float = Field(
        default=600.0, description="Default requests per minute"
    )
    default_requests_per_hour: float = Field(
        default=3600.0, description="Default requests per hour"
    )
...
class VideoProcessingConfig(BaseModel):
    ...
    video_quality: VideoQuality = Field(
        default=VideoQuality.HIGH, description="Default video quality"
    )
    default_video_format: VideoFormat = Field(
        default=VideoFormat.MP4, description="Default video format"
    )
    ...
    default_audio_format: AudioFormat = Field(
        default=AudioFormat.WAV, description="Default audio format"
    )
    default_audio_sample_rate: int = Field(
        default=44100, description="Default audio sample rate"
    )
    default_audio_bitrate: str = Field(
        default="128k", description="Default audio bitrate"
    )
...
class SecurityConfig(BaseModel):
    ...
    enable_api_key_validation: bool = Field(
        default=True, description="Enable API key validation"
    )
    enable_request_signing: bool = Field(
        default=False, description="Enable request signing"
    )
    api_key_rotation_days: int = Field(
        default=90, description="API key rotation period in days"
    )
...
class RetryConfig(BaseModel):
    ...
    default_retry_delay_seconds: float = Field(
        default=1.0, description="Default retry delay in seconds"
    )
    exponential_backoff_multiplier: float = Field(
...
class DevelopmentConfig(BaseModel):
    ...
    environment: Environment = Field(
        default=Environment.PRODUCTION, description="Environment type"
    )
    debug_mode: bool = Field(default=False, description="Enable debug mode")
    enable_mock_apis: bool = Field(
        default=False, description="Enable mock APIs for testing"
    )
    test_data_dir: Path = Field(
        default=Path("./test_data"), description="Test data directory"
    )
...
class ConfigurationManager:
    ...
    def _load_config_file(self, config_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            raise ConfigurationError(
                f"Failed to parse configuration file {config_file}: {str(e)}"
            ) from e
        ...
        return {
            ...
            "database": {
                "mongodb_connection_string": os.getenv(
                    ...
                    "mongodb://localhost:27017/video_system",
                ),
                "session_timeout_minutes": self._get_int_env(
                    "SESSION_TIMEOUT_MINUTES", 60
                ),
                "enable_session_encryption": self._get_bool_env(
            ...
            "storage": {
                ...
                "session_data_dir": Path(
                    os.getenv("SESSION_DATA_DIR", "./data/sessions")
                ),
            ...
            "logging": {
                ...
                "enable_structured_logging": self._get_bool_env(
                    "ENABLE_STRUCTURED_LOGGING", True
                ),
                "enable_audit_logging": self._get_bool_env(
                    "ENABLE_AUDIT_LOGGING", True
                ),
            },
            "performance": {
                "max_concurrent_requests": self._get_int_env(
                    "MAX_CONCURRENT_REQUESTS", 10
                ),
                "request_timeout_seconds": self._get_int_env(
                    "REQUEST_TIMEOUT_SECONDS", 300
                ),
                "max_memory_usage_mb": self._get_int_env("MAX_MEMORY_USAGE_MB", 4096),
                "enable_rate_limiting": self._get_bool_env(
                    "ENABLE_RATE_LIMITING", True
                ),
                "default_requests_per_second": self._get_float_env(
                    "DEFAULT_REQUESTS_PER_SECOND", 10.0
                ),
                "default_requests_per_minute": self._get_float_env(
                    "DEFAULT_REQUESTS_PER_MINUTE", 600.0
                ),
                "default_requests_per_hour": self._get_float_env(
                    "DEFAULT_REQUESTS_PER_HOUR", 3600.0
                ),
            },
            "video_processing": {
                ...
                "default_video_resolution": os.getenv(
                ...
                "default_audio_sample_rate": self._get_int_env(
                    "DEFAULT_AUDIO_SAMPLE_RATE", 44100
                ),
                "default_audio_bitrate": os.getenv("DEFAULT_AUDIO_BITRATE", "128k"),
            },
            "security": {
                "enable_api_key_validation": self._get_bool_env(
                    "ENABLE_API_KEY_VALIDATION", True
                ),
                "enable_request_signing": self._get_bool_env(
                    "ENABLE_REQUEST_SIGNING", False
                ),
                "api_key_rotation_days": self._get_int_env("API_KEY_ROTATION_DAYS", 90),
                "enable_circuit_breaker": self._get_bool_env(
                    "ENABLE_CIRCUIT_BREAKER", True
                ),
                "circuit_breaker_failure_threshold": self._get_int_env(
                    "CIRCUIT_BREAKER_FAILURE_THRESHOLD", 5
                ),
                "circuit_breaker_timeout_seconds": self._get_int_env(
                    "CIRCUIT_BREAKER_TIMEOUT_SECONDS", 60
                ),
            },
            "monitoring": {
                "enable_health_checks": self._get_bool_env(
                    "ENABLE_HEALTH_CHECKS", True
                ),
                "health_check_interval_seconds": self._get_int_env(
                    "HEALTH_CHECK_INTERVAL_SECONDS", 30
                ),
                "enable_performance_monitoring": self._get_bool_env(
                    "ENABLE_PERFORMANCE_MONITORING", True
                ),
                "enable_graceful_degradation": self._get_bool_env(
                    "ENABLE_GRACEFUL_DEGRADATION", True
                ),
            },
            "retry": {
                "default_max_retries": self._get_int_env("DEFAULT_MAX_RETRIES", 3),
                "default_retry_delay_seconds": self._get_float_env(
                    "DEFAULT_RETRY_DELAY_SECONDS", 1.0
                ),
                "exponential_backoff_multiplier": self._get_float_env(
    ...
    def _validate_api_keys(self) -> List[str]:
        ...
        for key_name, key_config in self._api_key_configs.items():
            ...
            if key_config.required and not env_value:
                issues.append(
                    f"Required API key missing: {key_name} - {key_config.description}"
                )
        ...
        if not any(os.getenv(key) for key in stock_apis):
            issues.append(
                "At least one stock media API key is required (Pexels, Unsplash, or Pixabay)"
            )
    ...
    def _validate_google_cloud_config(self) -> List[str]:
        ...
        if gcp_config.use_vertexai:
            ...
            if gcp_config.staging_bucket and not gcp_config.staging_bucket.startswith(
                "gs://"
            ):
...
def get_video_system_config() -> VideoSystemConfig:
    """Get the video system configuration."""
...
def get_system_config_summary() -> Dict[str, Any]:
