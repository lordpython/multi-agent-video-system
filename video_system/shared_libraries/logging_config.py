...
class VideoSystemLogger:
    ...
    def get_logger(self, name: str) -> logging.Logger:
        ...
        full_name = f"video_system.{name}" if not name.startswith("video_system") else name
...
class AuditLogger:
    ...
    def log_security_event(self, event: str, risk_level: str = "low", **kwargs):
        ...
        log_level = logging.WARNING if risk_level in ["medium", "high"] else logging.INFO
        self.logger.log(
            log_level,
...
def initialize_logging(log_level: str = "INFO", log_dir: Optional[str] = None):
    ...
    _performance_logger = PerformanceLogger(_video_system_logger.get_logger("performance"))
    _audit_logger = AuditLogger(log_dir)
    
    # Log initialization
    logger = _video_system_logger.get_logger("logging_config")
    logger.info(f"Logging system initialized with level: {log_level}, directory: {log_dir}")
...
def log_system_startup():
    ...
    startup_info = {
        ...
        "environment_variables": {
            ...
            if not key.endswith("_KEY") and not key.endswith("_SECRET")  # Don't log secrets
        }
    }
...
class LoggedOperation:
    ...
    def __init__(self, operation_name: str, logger: Optional[logging.Logger] = None, **kwargs):
