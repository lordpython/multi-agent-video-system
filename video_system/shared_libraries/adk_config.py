...
class ADKConfig(BaseSettings):
    ...
    session_cleanup_interval: int = Field(3600, description="Session cleanup interval in seconds")
    max_session_age_hours: int = Field(24, description="Maximum session age before cleanup")
...
class SessionServiceFactory:
    ...
    @staticmethod
    def create_session_service(config: Optional[ADKConfig] = None) -> BaseSessionService:
...
class ADKServiceManager:
    ...
    def __init__(self, config: Optional[ADKConfig] = None):
        ...
        logger.info(f"ADK Service Manager initialized with config: {self._get_config_summary()}")
    ...
    def get_session_service(self) -> BaseSessionService:
        ...
        if self._session_service is None:
            self._session_service = SessionServiceFactory.create_session_service(self.config)
        ...
        try:
            ...
            if self.config.use_vertex_ai:
                if not self.config.google_cloud_project:
                    logger.error("GOOGLE_CLOUD_PROJECT is required when using Vertex AI")
                    return False
                
                # Try to create a session service to validate credentials
                try:
                    session_service = SessionServiceFactory.create_session_service(self.config)
