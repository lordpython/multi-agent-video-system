...
class ResourceManager:
    ...
    def can_allocate_resources(self, 
                              cpu_cores: float = 0.0,
                              memory_mb: float = 0.0,
                              disk_mb: float = 0.0,
                              gpu_memory_mb: float = 0.0) -> Tuple[bool, str]:
        ...
        if cpu_cores > availability["cpu"]["available_cores"]:
            return False, f"Insufficient CPU cores: need {cpu_cores}, available {availability['cpu']['available_cores']}"
        
        # Check memory
        memory_gb_needed = memory_mb / 1024
        if memory_gb_needed > availability["memory"]["available_gb"]:
            return False, f"Insufficient memory: need {memory_gb_needed:.1f}GB, available {availability['memory']['available_gb']:.1f}GB"
        
        # Check disk
        disk_gb_needed = disk_mb / 1024
        if disk_gb_needed > availability["disk"]["free_gb"]:
            return False, f"Insufficient disk space: need {disk_gb_needed:.1f}GB, available {availability['disk']['free_gb']:.1f}GB"
    ...
    def force_garbage_collection(self) -> Dict[str, Any]:
        ...
        logger.info(f"Garbage collection freed {memory_freed:.1f}MB, collected {collected} objects")
    ...
    def _check_resource_alerts(self, usage: ResourceUsage):
        ...
        alerts_to_check = [
            (ResourceType.CPU, usage.cpu_percent, 
             self.thresholds.cpu_warning, self.thresholds.cpu_critical, self.thresholds.cpu_emergency),
            (ResourceType.MEMORY, usage.memory_percent,
             self.thresholds.memory_warning, self.thresholds.memory_critical, self.thresholds.memory_emergency),
            (ResourceType.DISK, usage.disk_percent,
             self.thresholds.disk_warning, self.thresholds.disk_critical, self.thresholds.disk_emergency)
        ]
        
        for resource_type, current_value, warning_threshold, critical_threshold, emergency_threshold in alerts_to_check:
            ...
            if alert_level:
                ...
                if alert_key in self.active_alerts and not self.active_alerts[alert_key].resolved:
                    ...
                    logger.info(f"Resource alert resolved: {resource_type.value.upper()} usage back to normal")
        ...
        try:
            ...
            if self.last_network_stats and self.last_network_time:
                ...
                if time_delta > 0:
                    ...
                    sent_mbps = (sent_delta * 8) / (time_delta * 1024 * 1024)  # Convert to Mbps
                    recv_mbps = (recv_delta * 8) / (time_delta * 1024 * 1024)  # Convert to Mbps
...
def initialize_resource_manager(thresholds: Optional[ResourceThresholds] = None) -> ResourceManager:
