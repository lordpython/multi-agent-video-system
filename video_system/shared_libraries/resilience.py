        ...
        if not health_check_func:
            ...
            try:
                ...
                if isinstance(check_result, bool):
                    status = ServiceHealth.HEALTHY if check_result else ServiceHealth.UNHEALTHY
                    details = {}
                elif isinstance(check_result, dict):
                    status = ServiceHealth(check_result.get("status", ServiceHealth.HEALTHY))
                ...
                result = HealthCheckResult(
                    ...
                    response_time_ms=response_time,
                    error_message=str(e),
                    details={"consecutive_failures": service_info["consecutive_failures"]}
                )
...
class ResourceMonitor:
    ...
    def get_current_metrics(self) -> ResourceMetrics:
        ...
        try:
            ...
            raise ResourceError(f"Failed to get resource metrics: {str(e)}", original_exception=e)
    ...
    def should_throttle_requests(self) -> bool:
        ...
        try:
            metrics = self.get_current_metrics()
            return (
                metrics.cpu_percent >= self.thresholds["cpu_warning"] or
    ...
    def acquire(self, tokens: int = 1) -> bool:
        ...
        with self._lock:
            ...
            if self.tokens >= tokens:
                ...
                self.logger.warning(f"Rate limit exceeded. Requested: {tokens}, Available: {self.tokens}")
                return False
...
class HealthMonitor:
    ...
    def _log_system_health(self, resource_status: Dict[str, Any]):
        ...
        healthy_services = sum(1 for h in service_health.values() if h.status == ServiceHealth.HEALTHY)
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "timestamp": datetime.utcnow().isoformat(),
                "overall_healthy": resource_status["healthy"] and all(
                    h.status == ServiceHealth.HEALTHY for h in service_health.values()
                ),
                "resource_status": resource_status,
                "service_health": {name: h.model_dump() for name, h in service_health.items()},
                "degradation_level": self.degradation.current_level,
