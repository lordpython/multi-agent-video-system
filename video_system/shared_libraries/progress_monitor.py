...
from .session_manager import SessionManager, SessionStage, get_session_manager
...
class ProgressMonitor:
    ...
    DEFAULT_STAGE_WEIGHTS = {
        ...
        SessionStage.RESEARCHING: 0.15,
        SessionStage.SCRIPTING: 0.20,
        SessionStage.ASSET_SOURCING: 0.25,
        SessionStage.AUDIO_GENERATION: 0.15,
        SessionStage.VIDEO_ASSEMBLY: 0.15,
        SessionStage.FINALIZING: 0.05
    }
    
    # Estimated durations for each stage (in seconds)
    DEFAULT_STAGE_DURATIONS = {
        SessionStage.INITIALIZING: 5,
        SessionStage.RESEARCHING: 30,
        SessionStage.SCRIPTING: 45,
        SessionStage.ASSET_SOURCING: 60,
        SessionStage.AUDIO_GENERATION: 40,
        SessionStage.VIDEO_ASSEMBLY: 90,
        SessionStage.FINALIZING: 10
    }
    ...
    def __init__(self, session_manager: Optional[SessionManager] = None):
        ...
        self.stage_progress: Dict[str, Dict[SessionStage, StageProgress]] = {}
        self.lock = threading.RLock()
        
        logger.info("ProgressMonitor initialized")
    
    def start_session_monitoring(self, session_id: str, 
                                stage_weights: Optional[Dict[SessionStage, float]] = None,
                                stage_durations: Optional[Dict[SessionStage, float]] = None) -> bool:
        ...
        with self.lock:
            ...
            for stage in SessionStage:
                if stage in [SessionStage.COMPLETED, SessionStage.FAILED]:
                    continue
    ...
    def update_stage_progress(self, session_id: str, stage: SessionStage, 
    ...
    def advance_to_stage(self, session_id: str, stage: SessionStage) -> bool:
    ...
    def complete_session(self, session_id: str, success: bool = True, 
        ...
        with self.lock:
            ...
            stage = SessionStage.COMPLETED if success else SessionStage.FAILED
...
def initialize_progress_monitor(session_manager: Optional[SessionManager] = None) -> ProgressMonitor:
...
def update_progress(session_id: str, stage: SessionStage, progress: float, details: Optional[str] = None) -> bool:
    """Update progress for a session stage."""
    return get_progress_monitor().update_stage_progress(session_id, stage, progress, details)


def advance_stage(session_id: str, stage: SessionStage) -> bool:
