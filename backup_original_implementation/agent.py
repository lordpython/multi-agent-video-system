...
from .shared_libraries.models import VideoGenerationRequest as VideoRequest
...
async def start_video_generation(
    ...
    try:
        ...
        if quality not in valid_qualities:
            error = ValidationError(
                f"Quality must be one of: {', '.join(valid_qualities)}", field="quality"
            )
            log_error(logger, error)
            return create_error_response(error)
        ...
        return {
            ...
            "status": session_state.model_dump()
            if session_state
        ...
        error = ProcessingError(
            f"Unexpected error starting video generation: {str(e)}",
...
def get_root_agent_tools():
    ...
    workflow_tools = [start_video_generation, execute_complete_workflow]
...
def initialize_video_system():
    ...
    logger.info(
        "Video system initialized with comprehensive error handling and resilience"
    )
...
def check_orchestrator_health() -> Dict[str, Any]:
    ...
    try:
        ...
        unhealthy_critical = [
            name
            for name, health in all_services.items()
