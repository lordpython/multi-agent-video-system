...
async def generate_video(
    request: VideoGenerationRequestAPI, background_tasks: BackgroundTasks
):
    ...
    try:
        ...
        if not result.get("success"):
            raise HTTPException(
                status_code=400,
                detail=result.get("error_message", "Failed to start video generation"),
            )
...
async def get_video_progress(
    ...
    try:
        ...
        if not progress_info:
            raise HTTPException(
                status_code=404, detail="Session not found or not being monitored"
            )
...
async def cancel_video_generation(
    ...
    try:
        ...
        progress_monitor.complete_session(
            session_id, success=False, error_message="Cancelled by user"
        )
...
async def list_video_sessions(
    ...
    status: Optional[str] = Query(
        None, description="Filter by status (completed, failed, processing, queued)"
    ),
    ...
    try:
        ...
        result = await session_manager.list_sessions_paginated(
            user_id=user_id, page=page, page_size=page_size, status_filter=status
        )
...
async def cleanup_sessions(
    max_age_hours: int = Query(24, ge=1, description="Maximum age in hours"),
):
...
async def _process_video_generation(session_id: str):
    ...
    try:
        ...
        logger.error(
            f"Error in background video generation for session {session_id}: {e}"
        )
        ...
        await session_manager.update_stage_and_progress(
            session_id, VideoGenerationStage.FAILED, error_message=str(e)
        )
        progress_monitor.complete_session(
            session_id, success=False, error_message=str(e)
        )
