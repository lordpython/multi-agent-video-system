...
class VideoGenerationDemo:
    ...
    async def generate_video_interactive(self):
        ...
        style = input("Enter video style (professional/casual/educational, default: professional): ").strip()
        if not style:
            style = "professional"
    ...
    async def generate_video_demo(self):
        ...
        for i, demo in enumerate(demo_prompts, 1):
            print(f"{i}. {demo['prompt'][:80]}... ({demo['duration']}s, {demo['style']})")
        
        choice = input(f"\nSelect a demo (1-{len(demo_prompts)}) or press Enter for demo 1: ").strip()
        try:
        ...
        await self.generate_video(
            selected_demo['prompt'],
            selected_demo['duration'],
            selected_demo['style']
        )
    ...
    async def generate_video(self, prompt: str, duration: int, style: str):
        ...
        try:
            ...
            if not start_result.get('success'):
                print(f"❌ Failed to start video generation: {start_result.get('error_message')}")
                return
            ...
            if workflow_result.get('success'):
                ...
                print(f"💡 Next steps: {workflow_result.get('error_message', 'Use coordination tools')}")
                print()
        ...
        for tool_name, description in coordination_steps:
            ...
            try:
                ...
                if result.get('success'):
                    ...
                    print(f"⚠️ {description} completed with issues: {result.get('error_message', 'Unknown error')}")
    ...
    async def call_agent_function(self, function_name: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
        """Call an agent function and return the result."""
        try:
            # Create a simple query that would trigger the function
            query = f"Please call {function_name} with parameters: {json.dumps(parameters)}"
...
def main():
    ...
    if not demo.check_environment():
        ...
        print("Please check your .env file and ensure all required APIs are configured.")
        print()
