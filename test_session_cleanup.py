...
async def test_session_cleanup():
    ...
    try:
        ...
        async with session_manager._registry_lock:
            user_id = session_manager.session_user_mapping.get(
                old_session_id, "anonymous"
            )
            if (
                user_id in session_manager.session_registry
                and old_session_id in session_manager.session_registry[user_id]
            ):
                old_time = (datetime.utcnow() - timedelta(hours=2)).isoformat()
                session_manager.session_registry[user_id][old_session_id][
                    "created_at"
                ] = old_time
                session_manager.session_registry[user_id][old_session_id][
                    "updated_at"
        ...
        completed_session_id = await session_manager.create_session(
            "Completed test session"
        )
        await session_manager.update_stage_and_progress(
            completed_session_id, VideoGenerationStage.COMPLETED, 1.0
        )

        # Make it appear old enough for cleanup
        async with session_manager._registry_lock:
            user_id = session_manager.session_user_mapping.get(
                completed_session_id, "anonymous"
            )
            if (
                user_id in session_manager.session_registry
                and completed_session_id in session_manager.session_registry[user_id]
            ):
                old_time = (datetime.utcnow() - timedelta(hours=2)).isoformat()
                session_manager.session_registry[user_id][completed_session_id][
                    "created_at"
                ] = old_time
                session_manager.session_registry[user_id][completed_session_id][
                    "updated_at"
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print(
                f"   ✓ Sessions eligible for cleanup: {cleanup_stats['sessions_eligible_for_cleanup']}"
            )
            print(
                f"   ✓ Estimated files for cleanup: {cleanup_stats['estimated_files_for_cleanup']}"
            )
            print(
                f"   ✓ Session age distribution: {cleanup_stats['session_age_distribution']}"
            )
            ...
            if pattern_files:
                pattern_cleanup = (
                    await session_manager.cleanup_session_files_by_pattern("*.tmp")
                )
                print(
                    f"   ✓ Pattern cleanup - Files cleaned: {pattern_cleanup['cleaned_files']}"
                )
                print(
                    f"   ✓ Pattern cleanup - Files failed: {pattern_cleanup['failed_files']}"
                )
            ...
            print(
                f"   ✓ Primary service available: {session_info.get('primary_service_available', False)}"
            )
            ...
            print(
                f"   ✓ Force cleanup completed in {force_cleanup_results['duration_seconds']:.2f}s"
            )
        ...
        if final_stats.average_completion_time:
            print(
                f"   ✓ Average completion time: {final_stats.average_completion_time:.2f}s"
            )
