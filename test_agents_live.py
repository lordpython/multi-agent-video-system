...
from video_system.tools.image_tools import generate_imagen_image
...
def test_imagen_generation():
    ...
    try:
        ...
        if result.get("total_images", 0) > 0:
            ...
            print(
                f"❌ Generation Error: {result.get('images', [{}])[0].get('error', 'Unknown error')}"
            )
...
def test_gemini_tts():
    ...
    try:
        result = generate_speech_with_gemini(
            text=text, voice_name="Zephyr", temperature=1.0, output_format="wav"
        )
        ...
        if result.get("total_files", 0) > 0:
            ...
            if audio.get("status") == "success":
                ...
                print(
                    f"⏱️ Estimated Duration: {audio.get('duration_estimate', 'N/A')} seconds"
                )
                ...
                if audio.get("audio_data"):
                    output_path = (
                        f"test_tts_output{audio.get('file_extension', '.wav')}"
                    )
                    with open(output_path, "wb") as f:
            ...
            print(
                f"❌ Generation Error: {result.get('audio_files', [{}])[0].get('error', 'Unknown error')}"
            )
    ...
    try:
        ...
        if result.get("status") == "success":
            ...
            for segment in result.get("timing_segments", []):
                print(
                    f"  Scene {segment.get('scene_number', 'N/A')}: "
